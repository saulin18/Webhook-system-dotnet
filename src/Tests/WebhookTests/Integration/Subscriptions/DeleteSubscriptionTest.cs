...
public sealed class DeleteSubscriptionTest : BaseIntegrationTest
{
    ...
    public DeleteSubscriptionTest(CustomWebApplicationFactory factory) : base(factory)
    {
    }
    ...
    public async Task DeleteSubscription_WithValidData_ReturnsSuccess()
    {
        ...
        var response = await Client.DeleteAsync(endpoint.Replace("{id}", subscription.Id.ToString()));
        response.EnsureSuccessStatusCode();
        Assert.True(response.IsSuccessStatusCode);
        var responseBody = await response.Content.ReadFromJsonAsync<DeleteSubscriptionResponseDto>();
